...
bool Model::initializeModel(const aiScene *scene, const string &filename)
{
    ...
    std::vector<cavr::math::vec3f> normals;
    std::vector<unsigned int> indices;
    std::vector<cavr::math::vec2f> textureCoords;
...
void  Model::initializeMesh(unsigned int index, const aiScene *scene, aiColor3D color, std::vector<cavr::math::vec3f>& positions, std::vector<cavr::math::vec3f>& normals, std::vector<unsigned int>& indices, std::vector<cavr::math::vec2f>& textureCoord)
{
    // initialize variables
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        float y = position->y;
        float z = position->z;
        positions.push_back(cavr::math::vec3f(x,y,z));
        ...
        if (mesh->HasTextureCoords(0))
        {
            ...
            x = t.x;
            y = t.y;
            textureCoord.push_back(cavr::math::vec2f(x,y));
        }
        else
        {
            textureCoord.push_back(cavr::math::vec2f(0, 0));
        }

        if (mesh->HasNormals()) {
            const aiVector3D* normal = &(mesh->mNormals[i]);
            normals.push_back(cavr::math::vec3f(normal->x, normal->y, normal->z));
        }
